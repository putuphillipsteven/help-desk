...
export default function WeekInput({ isGLobal }: WeekInputProps) {
	...
	return (
		<div className='dropdown flex flex-col items-center sm:items-end gap-y-8 h-full'>
			<div
				tabIndex={0}
				className={clsx(
					'px-2 py-1 sm:px-4 sm:py-2 cursor-pointer h-full centering-flex rounded-md',
					{
						'bg-primary text-white':
							getDiffDays(startDate || '', endDate || '') === 'week' && isGLobal,
					},
				)}
			>
				<p
					className={clsx('text-xs sm:text-sm', {
						'text-white': getDiffDays(startDate || '', endDate || '') === 'week' && isGLobal,
					})}
				>
					Week
				</p>
